...
- **多維度過濾條件**：可指定目標主機（Hosts）、目標端口（Ports）以及協議類型（TCP / UDP / ALL）。Hosts 欄位支援 **域名**（`*.google.com` 匹配任意子網域、`google.com` 匹配 apex）與 **IP**（如 `192.168.1.1`），多個目標以 `;` 分隔。域名規則由核心定期重新解析為 IP 後再比對（快取 TTL 1 分鐘）。
...
2. **填寫目標資訊**：
   ...
   - 範例（PID）：`1234`
3. **設定進階條件**：可選填目標主機（如 `*.google.com`）、端口（如 `443`）及傳輸協議（TCP/UDP/ALL）。Hosts 欄位支援 **域名**（`*.google.com` 透過 DNS 嗅探匹配 `mail.google.com` 等任意子網域；`google.com` 只匹配 apex）與 **IP**（如 `192.168.1.1`），多個目標以 `;` 分隔。萬用子網域比對依賴本機**明文 DNS（53 埠）**回應建立 IP→域名對照，DoH/DoT 或 DNS 本身走代理時該規則會退回 apex-only。
4. **選擇動作與代理**：
   - **代理**：指定透過某個已設定的自訂代理轉發。
   - **直連**：強制不經過代理，直接連線。
   - **阻擋**：攔截並阻斷該流量。
